...
interface CustomInternalAxiosRequestConfig extends InternalAxiosRequestConfig {
    _retry?: boolean;  // 재시도 여부를 나타내는 선택적 속성
}


// 전역 변수로 refresh 요청의 Promis를 저장해서 중복 요청을 방지
...
axiosInstance.interceptors.request.use((config) => {
    ...
    const accessToken = getItem();  // localStorage에서 accessToken 가져옴
...
axiosInstance.interceptors.response.use(
    ...
    async (error) => {
        ...
        if (
            ...
            // 이미 리프레시 요청이 진행 중인 경우, 해당 Promise 재사용
            if (!refreshPromise) {
                // refresh 요청 실행 후, 해당 Promise를 전역 변수에 할당
